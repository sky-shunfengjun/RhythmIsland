...
/// <summary>
/// 组件与配套背景共同使用的频谱视觉设置。该类型只包含与绘制有关的选项。
/// </summary>
public class SpectrumVisualSettings : ObservableObject, IJsonOnDeserialized
{
    ...
    private IReadOnlyList<SpectrumFrameRateOption> _availableFrameRates = SpectrumFrameRateOptions.ForRefreshRate(null);
    ...
    public int VisualizationStyleIndex
    {
        ...
        set => VisualizationStyle = value is >= 0 and <= 2 ? (SpectrumVisualizationStyle)value : SpectrumVisualizationStyle.Bars;
    }
    ...
    public bool? LegacyUseCustomColor { get => null; set => _legacyUseCustomColor = value; }
    ...
    public int MediaCoverColorModeIndex
    {
        ...
        set => MediaCoverColorMode = value is >= 0 and <= 2 ? (SpectrumMediaColorMode)value : SpectrumMediaColorMode.Vivid;
    }
    ...
    [JsonIgnore(Condition = JsonIgnoreCondition.WhenWritingNull)]
    public bool? LegacyGradientEnabled { get => null; set => _legacyGradientEnabled = value; }
    ...
    public bool GlowEnabled { get => _glowEnabled; set => SetProperty(ref _glowEnabled, value); }

    public double GlowIntensity
    {
        get => _glowIntensity;
        set => SetProperty(ref _glowIntensity, double.IsFinite(value) ? Math.Clamp(value, 0.10, 1.00) : 0.50);
    }

    public int FrameRate
    {
        get => _frameRate;
        set => SetProperty(ref _frameRate, AllowedFrameRates.Contains(value) ? value : 30);
    }

    public int BarCount
    {
        get => _barCount;
        set => SetProperty(ref _barCount, RhythmIslandSettings.AllowedBarCounts.Contains(value) ? value : 48);
    }

    public bool HorizontalMirrorEnabled { get => _horizontalMirrorEnabled; set => SetProperty(ref _horizontalMirrorEnabled, value); }
    ...
    public int FrequencyBalanceModeIndex
    {
        ...
        set => FrequencyBalanceMode = value is >= 0 and <= 2 ? (SpectrumFrequencyBalanceMode)value : SpectrumFrequencyBalanceMode.Balanced;
    }
    ...
    [JsonIgnore]
    public bool IsBottomUpMode { get => DisplayMode == SpectrumDisplayMode.BottomUp; set { if (value) DisplayMode = SpectrumDisplayMode.BottomUp; } }

    [JsonIgnore]
    public bool IsCenteredMode { get => DisplayMode == SpectrumDisplayMode.Centered; set { if (value) DisplayMode = SpectrumDisplayMode.Centered; } }

    [JsonIgnore] public bool IsThemeColorSource => ColorSource == SpectrumColorSource.ThemeAccent;
    [JsonIgnore] public bool IsMediaCoverColorSource => ColorSource == SpectrumColorSource.MediaCover;
    [JsonIgnore] public bool IsCustomColorSource => ColorSource == SpectrumColorSource.Custom;
    [JsonIgnore] public bool IsGradientActive => GradientMode != SpectrumGradientMode.Off;
    [JsonIgnore] public bool IsDynamicGradient => GradientMode == SpectrumGradientMode.Dynamic;
    [JsonIgnore] public bool CanCustomizeGradientEnd => IsGradientActive && !IsMediaCoverColorSource;
    [JsonIgnore] public string MediaCoverStatusText => _mediaCoverStatusText;

    internal void SetMediaCoverStatusText(string value) => SetProperty(ref _mediaCoverStatusText, value, nameof(MediaCoverStatusText));

    public void OnDeserialized()
    {
        if (!_hasExplicitColorSource && _legacyUseCustomColor.HasValue)
            _colorSource = _legacyUseCustomColor.Value ? SpectrumColorSource.Custom : SpectrumColorSource.ThemeAccent;
    ...
    internal virtual void Validate()
    {
        ...
        MediaCoverColorMode = MediaCoverColorMode;
        GradientMode = GradientMode;
        GradientSpeed = GradientSpeed;
        CustomColorText = CustomColorText;
        GradientEndColorText = GradientEndColorText;
        GlowIntensity = GlowIntensity;
        FrameRate = FrameRate;
        BarCount = BarCount;
        FrequencyBalanceMode = FrequencyBalanceMode;
        Amplitude = Amplitude;
        Opacity = Opacity;
    }
